...
public class Import_DetailDAO implements GenericDAO<Import_Detail_Note>{
	...
	public Import_Detail_Note findByName(String name) {
		Session session = sessionFactory.getCurrentSession();
		Import_Detail_Note importDetail = session.createQuery("FROM Import_Detail_Note WHERE import_Note_id='" + name + "'", Import_Detail_Note.class).getSingleResult();
		System.out.println(importDetail);
		return importDetail;
	}
